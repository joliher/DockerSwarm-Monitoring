...
# --- 2. DAR INTERNET A LOS WORKERS (NAT y FORWARD) ---
...
# --- 3. PROTEGER LOS CONTENEDORES (DOCKER-USER) ---
...
sudo iptables -I DOCKER-USER 1 -i $IF_LAN -p tcp --dport 80 -j RETURN
# Bloquear el resto del tráfico a contenedores que no sea del administrador
sudo iptables -I DOCKER-USER 2 -i $IF_LAN ! -s $IP_ADMIN -j DROP
